...
sed -i "s/boot.initrd.luks.devices.\"swap\".device = \".*\"/boot.initrd.luks.devices.\"swap\".device = \"\/dev\/disk\/by-uuid\/$(blkid $part2 -o value -s UUID)\"/g" /mnt/etc/nixos/configuration.nix
